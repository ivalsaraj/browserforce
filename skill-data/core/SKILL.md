---
name: core
description: Core BrowserForce usage guide. Read this before running any browserforce commands. Covers the snapshot-and-ref workflow against the user's real Chrome browser, the persistent session daemon (sessiond) atomic verbs (snapshot, click, fill, type, press, wait, get, eval), the one-shot -e Playwright path, taking screenshots, listing tabs, and troubleshooting connection problems. Use when the user asks to browse, fill a form, click something, extract data, take a screenshot, log into a site, or automate a browser task with their logged-in Chrome sessions.
...
BrowserForce has two execution paths. Prefer the **session daemon** for
step-by-step agent work; use **one-shot `-e`** for a self-contained script.

### 1. Session daemon (`--sessiond`) — persistent, atomic verbs (preferred)

A small localhost daemon holds one browser session so state (the current page,
snapshot refs, `state`) **persists across commands**. This is the natural fit
for the observe → act → observe loop.

```bash
browserforce snapshot --sessiond            # 1. accessibility tree + @eN refs
browserforce click @e3                        # 2. act on a ref from the snapshot
browserforce snapshot --sessiond            # 3. re-snapshot after the page changes
```
...
```bash
browserforce snapshot --sessiond [--selector "#main"] [--search "login"] [--interactive-only]
...
browserforce wait   --text "Saved"            # wait for text to appear
...
`eval --stdin` runs through the **same guarded execution boundary** as the MCP
`execute` tool — the snippet has `page`, `context`, `state`, `snapshot()`,
...
```bash
browserforce snapshot --sessiond                  # full accessibility tree
browserforce snapshot --sessiond --interactive-only  # interactive elements only
browserforce snapshot --sessiond --selector "#main"  # scope to a CSS selector
browserforce snapshot --sessiond --search "checkout" # only nodes matching text
browserforce snapshot 0                           # legacy one-shot snapshot of tab 0
```
...
browserforce navigate https://gmail.com  # open a URL in a new tab
```
...
```bash
browserforce navigate https://app.example.com/login
browserforce snapshot --sessiond            # find the email/password/submit refs
browserforce fill @e3 "user@example.com"
browserforce fill @e4 "hunter2"
browserforce click @e5
browserforce wait --url "**/dashboard"
browserforce snapshot --sessiond
```
...
4. **Session state persists with `--sessiond`** — one-shot `-e` does not.

## Troubleshooting

- **"Unknown ref" / element not found** — the page changed; re-run
  `browserforce snapshot --sessiond` and use the new refs.
